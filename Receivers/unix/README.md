...
## ScreamALSA (this repository)

The ScreamALSA kernel driver (`snd-screamalsa.c` in the repository root) streams audio
over UDP/TCP using the Scream protocol. Pair it with this receiver as follows:

| Setting   | ScreamALSA driver default | Receiver flag        |
|-----------|---------------------------|----------------------|
...
Configure the driver target IP/port in `scream.conf` (see root `scream_config.sh`).
The standard Scream default port is 4010 (multicast); ScreamALSA uses 4011 (unicast).
